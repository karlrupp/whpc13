...
int main(int argc,char **argv)
{
  ...
  if (alloc_star) {
    Mat B;
    ierr = DMCreateMatrix(dmstar,PETSC_NULL,&B);CHKERRQ(ierr);
  ...
  if (myJ) { ierr = DMDASNESSetJacobianLocal(dm,(PetscErrorCode (*)(DMDALocalInfo*,void*,Mat,Mat,MatStructure*,void*))FormJacobianLocal,(void *)&user);CHKERRQ(ierr); }
